...
* 并发
  + [[https://stackoverflow.com/questions/10589103/concurrenthashmap-locking][java - ConcurrentHashMap locking - Stack Overflow]]
...
* 初始化
  + 对象：创建对象 -> 填充零值 -> <init>(super.<init> -> block)
  + 类型：加载类型 -> 准备阶段（零值或常量） -> <clinit>(super.<clinit> -> static block)
