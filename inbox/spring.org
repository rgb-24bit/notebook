...
* Filter
  ...
  + 当前过滤器持有的 AuthenticationManager 认证失败，下一个过滤器会进行认证，最后是怎么判断是否认证成功的
  + 拦截器
  + 好像是在 FilterSecurityInterceptor 完成最后的判断，即：前面的过滤器尝试验证，不管是否成功，最后进行权限判断的时候进行处理，那么，
    存在多个认证的时候是怎样处理的？有一个认证成功后后续又怎么处理的？
